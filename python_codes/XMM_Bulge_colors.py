"""Python programs to compare colors of different sources."""
...
def plot_spec_summary(ebins, sim_src_spec, sim_bg_spec, obs_src_spec=None,
                      obs_bg_spec=None, det='', en_range=None, plot=True,
                      det_mask_sim=None, det_mask_obs=None):
    """Plot spec summary."""
    if en_range is None:
        en_range = [0.2, 10.0]
    ...
    if plot is False:
        ...
        return (net_counts_sim, bg_counts_sim, en_lowindex, en_highindex)
    if obs_src_spec is not None:
        plotline_scatter([ebins[en_lowindex:en_highindex],
                          ebins[en_lowindex:en_highindex]],
                         [median_filter(np.mean(norm_spec_sim, axis=0),
                                        size=10),
                          median_filter(np.mean(norm_spec_obs, axis=0),
                                        size=10)],
                         xlabel='Energy (keV)',
    ...
    plotline_scatter([ebins[en_lowindex:en_highindex]],
                     ...
                     xlabel='Energy (keV)',
                     ylabel='Mean smoothed normalized spectra',
                     pl_labels=['Simuated MSPs'],
                     title='Simulated and Observed ' + det + ' spectra')
    plot_subplots(
        2, 2, [[[np.log10(net_counts_sim[det_mask_sim])],
                [np.log10(bg_counts_sim/net_counts_sim)[det_mask_sim]]],
               [np.log10(net_counts_sim[det_mask_sim]),
                np.log10(bg_counts_sim[det_mask_sim])]],
        [[None, None], [np.log10(bg_counts_sim[det_mask_sim]), None]],
        title='Net, Bg counts in simulated ' + det + ' spectra',
        subp_types_arr=[['hist', 'hist'], ['hist', 'hist']],
        pl_types_arr=[['withkde', 'withkde'], ['withkde', 'withkde']],
        xlabel_arr=[['log(Net counts)', 'log(Bg counts/Net counts)'],
                    ['log(Simulated net counts)', 'log(Bg counts)']],
        ylabel_arr=[['# per bin', '# per bin'], ['log (Bg counts)',
                                                 '# per bin']],
        pl_labels_arr=[['Simulated MSPs', 'Simulated MSPs'], 
                       [None, 'Simulated MSPs']])
...
def process_onedet_one_spectype(det_ebins, det_srcspecs, det_bgspecs, det_mask,
                                det_name='', det_obs_ebins=None,
                                det_obs_srcspecs=None, det_obs_bgspecs=None,
                                det_obsmask=None, range_fe=None):
    """Process spectra from one detector and one type of source."""
